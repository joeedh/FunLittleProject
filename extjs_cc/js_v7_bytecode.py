...
def parse_opdef():
  global opdef, mnomic_override
  
  lines = [l.strip() for l in opdef.split("\n")]
  i = 0;
  ...
  def gen_mnomic(code):
    ...
    if code in mnomic_override:
      ret = mnomic_override[code]
    elif "_" in code:
      code = code.split("_")
      for c in code:
        ret += c[0].lower()
  ...
  for l in lines:
    ...
    if "OP_" not in l: continue
    
    long = l[l.find("OP_"):l.find(",")].strip()
  ...
  return ops, opmap, mnomics

ops, opmap, mnomics = parse_opdef()
